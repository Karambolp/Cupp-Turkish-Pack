...
def print_to_file(filename, unique_list_finished):
    ...
    f.close()
    print(
        "[+] Kaydediliyor \033[1;31m"
        + filename
        + "\033[1;m, Sayı \033[1;31m"
        + str(lines)
        + " words.\033[1;m"
    )
    inspect = input("> Oluşturulan Şifreler Terminal Ekranında Gösterilsinmi? (E/H) : ").lower()
    if inspect == "e":
        try:
    ...
    print(
        "[+] Şimdi tabancanızı şununla yükleyin: \033[1;31m"
        + filename
        + "\033[1;Ve Ateşleyin! İyi Şanslar!"
    )
...
def print_cow():
    ...
    print(
        "           \033[1;31m   ||--|| \033[1;m\033[05m*\033[25m\033[1;m      [ Yapımcı: Muris Kurgas | https://github.com/Mebus ]"
    )
    print(28 * " " + "[Türkçeleştiren: Karambol | https://github.com/Pkarambols]\r\n")
...
def improve_dictionary(file_to_open):
    ...
    print("[+] Liste Sıralanıyor Kopyalar Kaldırılıyor...")
...
def interactive():
    ...
    print("\r\n[+] Bir word list oluşturmak için kurbanla ilgili bilgileri girin")
    print("[+] Bilmediğiniz bir sorunun cevabını geçmek için enter tuşuna basmanız yeterlidir;)\r\n")
    ...
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] Bana kurbanın ismini bilmediğini sakın söyleme!")
        name = input("> İsim: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> Soyad: ").lower()
    profile["nick"] = input("> Takma İsim: ").lower()
    birthdate = input("> Doğum Tarihi (GGAAYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] Doğum Tarihi için 8 hane girmelisiniz.!")
        birthdate = input("> Doğum Tarihi (GGAAYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> Eşinin İsmi: ").lower()
    profile["wifen"] = input("> Eşinin Takma İsmi: ").lower()
    wifeb = input("> Eşinin Doğum Tarihi (GGAAYYYY): ")
    ...
    profile["kid"] = input("> Çocuğunun İsmi: ").lower()
    profile["kidn"] = input("> Çocuğunun Takma İsmi: ").lower()
    kidb = input("> Çocuğunun Doğum Tarihi (GGAAYYYY): ")
    ...
    profile["pet"] = input("> Hayvanının İsmi: ").lower()
    profile["company"] = input("> Şirketinin İsmi: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> Kurban hakkında bazı anahtar kelimeler eklemek ister misiniz?? E/[H]: "
    ).lower()
    words2 = ""
    if words1 == "e":
        words2 = input(
            "> Lütfen Kelimeler Arasına Virgül İşareti Bırakınız. [nissan,gtr,galatasaray,siyah], Boşuklar Kaldırılacaktır: "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> Kelimelerin sonuna özel karakterler eklemek istiyor musunuz[Şifrenin Bulunma İhtimali Yükselecektir]? E/[H]: "
    ).lower()

    profile["randnum"] = input(
        "> Kelimelerin sonuna bazı rasgele sayılar eklemek ister misiniz [Şifrenin Bulunma İhtimali Yükselecektir]? E/[H]:"
    ).lower()
    profile["leetmode"] = input("> Leet Modu Aktif Edilsinmi? (i.e. leet = 1337) E/[H]: ").lower()
...
def get_parser():
    ...
    group.add_argument(
        ...
        "--interactive",
        action="store_true",
        help="Word List Oluştururken Detaylı Sorular Sorar.",
    )
    group.add_argument(
        "-w",
        dest="improve",
        metavar="FILENAME",
        help="Mevcut sözlüğü iyileştirmek için bu seçeneği kullanın,"
        " veya bazı pwnsauce yapmak için WyD.pl çıktısı",
    )
    group.add_argument(
        "-l",
        dest="download_wordlist",
        action="store_true",
        help="Depoda Bulunan Hazır Wordlist'leri İndirir",
    )
    group.add_argument(
        "-a",
        dest="alecto",
        action="store_true",
        help="Varsayılan kullanıcı adlarını ve şifreleri doğrudan ayrıştırmaya yarar"
    )
    group.add_argument(
        "-v", "--version", action="store_true", help="Versiyon Kontrolü Yapmanıza Yarar."
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Sessiz Modu Aktifleştirir"
    )
